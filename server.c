        ...
        else if(FD_ISSET(con_socket,&descp_set))
         {
            ...
            if(strncmp(con_msg,"ls",2)==0)
            {
             // err_check=send(con_socket,&con_msg,sizeof(con_msg),0);
              ...
              count_file = scandir("./server_dir", &entry_list,0,alphasort);
              if (count_file <0) 
              {
                 perror("scandir");
                 return EXIT_FAILURE;
            ...
            { 
              //printf("server-get %s\n",con_msg);
              sscanf(con_msg, "%s %s", cmd, file_name);  
              strcpy(full_pathname, server_path);
              strcat(full_pathname, file_name);
              //printf("full_pathname %s, server_path %s, file_name %s\n", full_pathname, server_path, file_name);
...
int check_add_user(char *newuser)
{
    ...
    sscanf(newuser, "%s", newname); 
    if( (file_dir = fopen ("users.dat", "a+")) < 0 )
	{
		perror("failed to open input file\n");
		return 1;
    } 
